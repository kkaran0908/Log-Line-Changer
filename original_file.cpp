...
void adl_algo_bin::process_external_request (const std::shared_ptr<AlgoJobRequestData>& data)
{
    ...
    ALGO_ILOG(
        "[algo:%s]: process_external_request=[%s], state=%s",
        inst_id().to_string(),
    ...
    switch(request->id)
    {
        case algojob__request_recovery:
        {
            ...
            {
                ALGO_ELOG("[algo:%s]: ADL Recovery is not supported - setting algo_state to [failed] and cleaning up child orders",
                          inst_id().to_string());
        ...
        case algojob__request_deploy_instr:
        {
            ...
            if (algo_version[0] == '\0')
            {
                ...
                ALGO_ELOG("[algo:%s] Request is missing algo version.", inst_id().to_string());
            ...
            ALGO_DLOG("[algo:%s] EPIQ_flag=%d", inst_id().to_string(), epiqFlag);
        ...
        case algojob__request_export_value_snapshot:
            ...
            {
                ALGO_DLOG("Algo has no Export Value. Ignore reuqest=%s", ALGOJOB_REQUEST_ID_STR[request->id]);
            }
            return;
        case algojob__requets_server_disconnected:
            ALGO_DLOG("[algo:%s] Detected server disconnect but will ignore it!", inst_id().to_string());
            return;
        ...
        case algojob__request_set_oma_parent_id:
            ...
            ALGO_ILOG(
                      "[algo:%s] Updated OMA Parent ID on request: "
                      "request=%s "
                      "new_oma_parent_id=%s",
                      inst_id().to_string(),
                      ALGOJOB_REQUEST_ID_STR[request->id],
        ...
        case algojob__request_clear_oma_parent_id:
            ...
            TTSDK_ClearOMAParentOrderId(CastIntParamsToExt(&m_algo_params));
            ALGO_ILOG(
                      "[algo:%s] Cleared OMA Parent ID on request: "
                      "request=%s",
                      inst_id().to_string(),
                      ALGOJOB_REQUEST_ID_STR[request->id]
                     );
        ...
        case algojob__request_order_pass:
        {
            ...
            {
                ALGO_ELOG("[algo:%s] algojob__request_order_pass: request_params was a nullptr",
                          inst_id().to_string());
                return reject_request(request);
            }
            ...
            ALGO_ILOG("[algo:%s] algojob__request_order_pass: Attempting to update params with: "
                      "obp_state=%s "
                      "current_group_id=%llu "
                      "pass_to_group_id=%llu "
                      "previous_pass_to_group_id=%llu "
                      "original_group_id=%llu",
                      inst_id().to_string(),
                      std::string(OrderPassToString(obp_state)),
                      current_group_id,
                      pass_to_group_id,
            ...
            if (state != algojob__algo_started &&
                state != algojob__algo_paused)
            {
                ALGO_ELOG("[algo:%s] algojob__request_order_pass: Not allowed in current state=%s",
                          inst_id().to_string(),
                          std::string(ALGO_STATE_STR[state]));
                return reject_request(request);
            }

            if (obp_state == order_pass_none)
            {
                ALGO_ELOG("[algo:%s] algojob__request_order_pass: Missing OBP state!",
                          inst_id().to_string());
                return reject_request(request);
            }
            ...
            if (obp_state == order_pass_accept || obp_state == order_pass_set_child)
            {
                ...
                {
                    ALGO_ELOG("[algo:%s] algojob__request_order_pass: Missing current_group_id for OBP state=%s!",
                              inst_id().to_string(),
                              std::string(OrderPassToString(obp_state)));
                    return reject_request(request);
                }
                ...
                if (m_algo_inst_mgr == nullptr)
                {
                    ALGO_ELOG("[algo:%s] algojob__request_order_pass: Missing AlgoInstMgr for OBP state=%s!",
                              inst_id().to_string(),
                              std::string(OrderPassToString(obp_state)));
                    return reject_request(request);
                }
            ...
            if (obp_state == order_pass_accept || obp_state == order_pass_set_child)
            {
                ...
                ALGO_ILOG("[algo:%s] algojob__request_order_pass: Set current and future child orders with %s",
                    inst_id().to_string(), GroupIDsToString(group_ids));
                m_algo_inst_mgr->OnCurrentUserGroupIdChanged(group_ids);
            }

            ALGO_ILOG("[algo:%s] algojob__request_order_pass: Success!", inst_id().to_string());
            server_connection::instance().send_request_complete(
                request->id,
                request->flags,
                request->user_request_id,
...
void adl_algo_bin::init_action (algoif::AlgoAction action)
{
    ...
    {
        ALGO_ILOG("[algo:%s] queue up action=%s. state=%s",
                  inst_id().to_string(),
                  ALGO_ACTION_STR[ action],
                  ALGO_STATE_STR[m_host_if->get_state()]);
...
void adl_algo_bin::process_host_event (const std::shared_ptr<void>& data)
{
    ...
    case host_event_id::compilation_failed:
        ...
        case host_event_id::signal_connector:
        {
            ...
            {
                 ALGO_ILOG("[algo:%s] signaler_id=%llu is stopped, dropped this event",
                           inst_id().to_string(),
                           event->signaler_id);
            }
            else
        ...
        case host_event_id::exit_block_graph:
        {
            ...
            ALGO_ILOG("[algo:%s] now process scheduled exit_block_graph m_globalVIndex=%llu", inst_id().to_string(), event->m_globalVIndex);
...
{
    auto state = m_state.load(std::memory_order_relaxed);
    ALGO_ILOG("[algo:%s]: process_compilation_completed_event: "
              "state=%s "
              "compile_reason=%s",
              inst_id().to_string(),
              ALGO_STATE_STR[state],
              instr_compile_reason_to_string(reason));
    ...
    try
    {
        ...
        ALGO_ILOG("[algo:%s]: hasAnalytics=%d analyticsBlockCount=%d",
                  inst_id().to_string(),
                  (dl()->algo_graph.analyticsBlockCount > 0), //  keeping the boolean hasAnalytics for log-level compatibility
        ...
        ALGO_DLOG("[algo:%s]: Creating AlgoInstMgr with %s",
                  inst_id().to_string(), GroupIDsToString(group_ids));
        ...
        if (reason == instr_compile_recovered)
        {
            ...
            ALGO_ILOG("[algo:%s]: process_compilation_completed_event: "
                      "Asynchronously download child orders for recovery before continuing.",
                      inst_id().to_string());
            recover_children();
            return;
        }
...
{
    auto state = m_state.load(std::memory_order_relaxed);
    ALGO_ILOG("[algo:%s]: create_block_graph_and_init_action: "
              "Continue processing compilation completed event: "
              "state=%s "
              "compile_reason=%s",
              inst_id().to_string(),
              ALGO_STATE_STR[state],
    ...
    try
    {
        ...
        *(m_root_block_graph.get()) = m_inst_if->CreateBlockGraph(root_def_block,
        ...
        ALGO_ILOG("[algo:%s]: m_has_periodic_task=%d HasExportValue=%d hasAlert=%d hasPnL=%d",
                    inst_id().to_string(),
                    m_has_periodic_task,
                    m_inst_if->HasExportValue(),
                    dl()->algo_graph.hasAlert,
                    m_algo_inst_mgr->m_hasPnl);
        ...
        {
            ALGO_ILOG("[algo:%s]: will use account_id=%llu",inst_id().to_string(), accountId);
        }
...
void adl_algo_bin::process_action_failed_event ()
{
    ...
    ALGO_ILOG("[algo:%s] process_action_failed_event: "
              "current_synth_status=%s "
              "next_synth_status=%s "
              "current_algo_state=%s "
              "next_algo_state=%s "
              "(note: current and next algo_state are expected to be both algojob__algo_failed) "
              "failed_action=%s",
              inst_id().to_string(),
              TTSDK_SYNTH_STATUS_STR[current_synth_status],
              TTSDK_SYNTH_STATUS_STR[next_synth_status],
              ALGO_STATE_STR[current_algo_state],
              ALGO_STATE_STR[next_algo_state],
              ALGOJOB_REQUEST_ID_STR[failed_action]);
    ...
    {
        ALGO_ILOG("[algo:%s] process_action_failed_event: Skipping purge child orders to allow recovery to be retried.",
                  inst_id().to_string());
        return;
    }
...
void adl_algo_bin::process_action_complete_event ()
{
    ...
    auto state = m_state.load(std::memory_order_relaxed);
    ALGO_ILOG("[algo:%s] process_action_complete_event:action=%s state=%s,",
              inst_id().to_string(),
              ALGO_ACTION_STR[action],
              ALGO_STATE_STR[state]);
...
void adl_algo_bin::suppress_action ()
{
    ...
    if (state == algojob__algo_started)
    {
        ALGO_ILOG("[algo:%s] suppress action", inst_id().to_string());
        if (m_host_if)
        {
...
void adl_algo_bin::save_state_to_disk ()
{
    if (m_host_if)
    {
        ALGO_ILOG("[algo:%s] Saving state to disk.", inst_id().to_string());
...
{
    ALGO_ILOG("[algo:%s] recover_children: Downloading children.", inst_id().to_string());
    ...
    {
        delete_pr = true;
        ALGO_ILOG("[algo:%s] recover_children: Will attempt to delete PR child order (if any) because: "
                  "is_sse_pr_enabled=%s",
                  inst_id().to_string(),
                  env::instance().is_sse_pr_enabled() ? "true" : "false");
    }
...
{
    auto state = m_state.load(std::memory_order_relaxed);
    ALGO_ILOG("[algo:%s] on_children_recovered: "
              "state=%s "
              "result=%s "
              "children_count=%zu",
              inst_id().to_string(),
    ...
    if (!result)
    {
        ALGO_ELOG("[algo:%s] on_children_recovered: Download failed, declaring recovery failure!", inst_id().to_string());
        process_compilation_failed_event(algojob__request_failure_to_recover_child_orders, "Failed to recover children");
        return;
    }

    ALGO_ILOG("[algo:%s] on_children_recovered: Download success, printing recovered child order IDs.", inst_id().to_string());
    for (size_t i = 0; i < size; i++)
    {
        const ttsdk_params child_param = params[i];
        if (child_param == nullptr)
        {
            ALGO_ELOG("[algo:%s] on_children_recovered: Param for a recovered child was unexpectedly a nullptr: "
                      "index=%zu",
                      inst_id().to_string(),
                      i);
            continue;
        ...
        {
            ALGO_ELOG("[algo:%s] on_children_recovered: Order ID missing for a recovered child: "
                      "index=%zu",
                      inst_id().to_string(),
                      i);
            continue;
